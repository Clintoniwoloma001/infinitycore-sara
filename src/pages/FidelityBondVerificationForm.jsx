...
import {
  ...
  Loader2, Send, Trash2, Upload, XCircle, Camera, PenTool, Shield, Home, User,
} from 'lucide-react'
...
export default function FidelityBondVerificationForm() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setStatus('loading')
      try {
        const data = await fidelityBondVerificationService.getDetails(token)
        if (!active) return
        setDetails(data)
        setForm({
          phone: data.phone || '',
  ...
  const uploadDoc = async (file, docType) => {
    ...
    setUploading(true)
    try {
      const meta = await fidelityBondVerificationService.uploadDocument({ token, file, documentType: docType })
  ...
  const doSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      const result = await fidelityBondVerificationService.submit(token, payload)
      setDone(result)
    } catch (e) {
  ...
  const stepContent = useMemo(() => {
    if (step === 1) return (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Surety Full Name">
          <input className={`${inputCls} bg-slate-50`} value={details?.surety_name || ''} readOnly />
        </Field>
        <Field label="Email">
          <input className={`${inputCls} bg-slate-50`} value={details?.surety_email || ''} readOnly />
        </Field>
        <Field label="Relationship to Employee">
          <input className={`${inputCls} bg-slate-50`} value={details?.surety_relationship || ''} readOnly />
        </Field>
        ...
        <Field label="Occupation" required>
          <input className={inputCls} value={form.occupation || ''} onChange={set('occupation')} placeholder="e.g. Business Owner" />
        </Field>
  ...
  if (status === 'submitted' || done) return (
    <Shell>
      <div className="text-center py-16">
        ...
        <h2 className="text-xl font-semibold text-slate-900">{done ? 'Verification Submitted' : 'Already submitted'}</h2>
        <p className="text-sm text-slate-500 mt-2">Your fidelity bond verification has been submitted and is pending HR review. Contact HR if you need to make changes.</p>
      </div>
    </Shell>
  )
    ...
    <Shell>
      {details && (
        <div className="mb-6 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          <p className="font-medium">Fidelity Bond Verification for {details.employee_name || 'Employee'}</p>
      ...
      <header className="bg-[#0a0b0d] text-white">
        <div className="max-w-3xl mx-auto px-4 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-semibold">Fidelity Bond Verification</div>
              <div className="text-xs text-white/50">InfinityCore Human Resources</div>
            </div>
          </div>
          <Shield className="text-[#009944]" />
        </div>
      </header>
