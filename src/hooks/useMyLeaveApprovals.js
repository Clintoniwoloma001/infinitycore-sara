...
import { myQueue as computeMyQueue, currentStage, pendingAgeHours, loadApprovalChainsForRequests } from '../services/leaveApprovalsService'
...
export function useMyLeaveApprovals() {
  ...
  const refresh = useCallback(async () => {
    if (!user) { setAll([]); setChains({}); setLoading(false); return }
    try {
      const data = await svc.list()
      setAll(data)
      try {
        setChains(await loadApprovalChainsForRequests(data))
      } catch {
        setChains({})
      }
      setError(null)
  ...
  const queue = computeMyQueue(all, { userId: user?.id, role, isAdmin }, chains)
  ...
  return {
    ...
    stageLabel: (r) => currentStage(r, chains[r.id])?.label,
    refresh,
  }
}
