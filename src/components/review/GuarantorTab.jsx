...
// Mirrors DOC_TYPES in GuarantorVerificationForm.jsx — the source of truth.
const DOC_TYPES = [
          ...
          <SubSection title="Guarantor Documents" icon={FileText}>
            {guarantorDocuments.length === 0 ? (
              ...
              <div className="space-y-3">
                {DOC_TYPES.map((dt) => {
                  const shotDocs = guarantorDocuments.filter((doc) => doc.document_type === dt.key)
                ...
                {guarantorDocuments.some((doc) => !DOC_TYPES.some((dt) => dt.key === doc.document_type)) && (
                  <div className="rounded-lg border border-slate-200 p-3">
                    <p className="text-sm font-medium text-slate-700 mb-2">Other documents</p>
                    <div className="space-y-2">
                      {guarantorDocuments.filter((doc) => !DOC_TYPES.some((dt) => dt.key === doc.document_type)).map((doc) => (
                        <div key={doc.id} className="flex items-center justify-between rounded-lg border border-slate-100 bg-slate-50 p-2.5">
                          <div className="flex items-center gap-3">
                            <FileText className="w-4 h-4 text-slate-400" />
                            <div>
